...
int splitPosition = 0, order = 0, arrayLocalSize = 0;
...
void moveItemsInOrder(int rank, MPI_Comm communicator) {
    ...
    int recvSize;
    MPI_Sendrecv(&sendSize, 1, MPI_INT, neighbour, SIZE, &recvSize, 1, MPI_INT, neighbour, SIZE, communicator,
                 MPI_STATUS_IGNORE);

    int sizeTemp = recvSize + arrayLocalSize - sendSize;
    int *newArray = new int[sizeTemp];
    MPI_Sendrecv(arrayLocal + sendFrom, sendSize, MPI_INT, neighbour, ARRAY, newArray, recvSize, MPI_INT, neighbour,
                 ARRAY, communicator, MPI_STATUS_IGNORE);
