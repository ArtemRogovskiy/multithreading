...
//    MPI_Status status;
    ...
    A_bloc = new double[bloc_size * m];
    F_bloc = new double[bloc_size];
    X_bloc = new double[bloc_size];

    /*......Scatter the Input Data to all process ......*/
    MPI_Scatter (A, bloc_size * m, MPI_DOUBLE, A_bloc, bloc_size * m,
                 MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatter (F, bloc_size, MPI_DOUBLE, F_bloc, bloc_size, MPI_DOUBLE, 0,
                 MPI_COMM_WORLD);

    auto start = std::chrono::high_resolution_clock::now();

    jacobi(m, A, F, X, bloc_size, rank, A_bloc, F_bloc, X_bloc);
